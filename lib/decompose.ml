(** Closure decomposition: scope analysis -> Worker groups -> IR.
    ...
    Algorithm: 1. Seed groups from mutable state: functions that capture the
    same mutable variable are placed in the same group. 2. Remaining functions
    (no mutable captures) become their own group. 3. Cross-group function calls
    become service binding dependencies. 4. Convert groups to IR Workers with KV
...
let find_group_of_function (groups : worker_group list) (fn_name : string) :
    string option =
  List.find_map
    (fun g -> if List.mem fn_name g.functions then Some g.name else None)
    groups

(** Derive a group name from the mutable state variable name. e.g. "store" ->
    "storage", "cache" -> "cache" *)
...
let decompose (analysis : Scope.analysis_result) : worker_group list =
  ...
  let all_mutable =
    String_list.dedup_preserve_order
      (List.concat_map
        ...
        let fns =
          List.filter_map
            (fun (c : Scope.closure_info) ->
              if List.mem state_var c.captures_mutable then Some c.name
              else None)
            closures
        in
        (state_var, String_list.dedup_preserve_order fns))
      all_mutable
  in
    ...
    List.fold_left
      (fun acc (state_var, fns) ->
        ...
        | _ ->
            let all_states =
              String_list.dedup_preserve_order
                ([ state_var ] @ List.concat_map fst overlapping)
            in
            let all_fns =
              String_list.dedup_preserve_order
                (fns @ List.concat_map snd overlapping)
            in
    ...
    List.map
      (fun (states, fns) ->
        let name =
          ...
          | _ -> String.concat "_" (List.map group_name_of_state states)
        in
        { name; functions = fns; owned_state = states; service_deps = [] })
      merged_groups
  in
        ...
        List.concat_map
          (fun fn_name ->
            ...
            | Some closure ->
                List.filter_map
                  (fun called ->
                    match find_group_of_function all_groups called with
                    | Some target_group when target_group <> group.name ->
                        Some target_group
                    | _ -> None)
                  closure.called_functions)
          group.functions
      in
      { group with service_deps = String_list.dedup_preserve_order deps })
    all_groups
...
let generate_worker_script (group : worker_group) : string =
  let env_types =
    List.map
      (fun state_var ->
        Printf.sprintf "  %s: KVNamespace;" (String.uppercase_ascii state_var))
      group.owned_state
    @ List.map
        (fun dep ->
          Printf.sprintf "  %s: Fetcher;" (String.uppercase_ascii dep))
        group.service_deps
  in
  let env_type_block =
    if env_types = [] then ""
  ...
  let svc_access =
    List.map
      (fun dep ->
        Printf.sprintf "  const %s = env.%s;" dep (String.uppercase_ascii dep))
      group.service_deps
  in
  let body_lines =
    ...
    @ List.map
        (fun fn_name -> Printf.sprintf "  // handler: %s" fn_name)
        group.functions
    @ [ "  return new Response(\"ok\");" ]
  in
  let env_param = if env_types = [] then "_env" else "env" in
  Printf.sprintf
    "%sexport default {\n\
    ...
    env_type_block env_param
    (String.concat "\n" body_lines)

(** Convert worker groups to IR config. *)
let to_ir (groups : worker_group list) (analysis : Scope.analysis_result) :
    Ir.config =
  List.map
    (fun group ->
      let kv_bindings =
            ...
            Ir.KVBinding
              {
                ...
                namespace_id = Printf.sprintf "cloudlang-%s" state_var;
              })
          group.owned_state
      in
      let svc_bindings =
      ...
      let routes =
        ...
        let exported =
          List.filter (fun fn -> List.mem fn analysis.exports) group.functions
        in
        match exported with
        | [] -> []
